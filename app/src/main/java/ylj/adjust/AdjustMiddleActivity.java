...
import ylj.common.widget.PlotView.DrawEdit;
...
public class AdjustMiddleActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_adjust_middle);
		...
		testCtrl.addAdjustHandler(handler);
		...
		mBuilder.setPositiveButton(getString(R.string.ad_ok), new DialogInterface.OnClickListener() {				
			...
			{
				AdjustMiddleActivity.this.onStopButton();
				dialog.dismiss();
				AdjustMiddleActivity.this.finish();
			}
		});
	...
	private void refresh()
	{
		...
		quakeDatas[currentIndex]=testCtrl.getCommonData().getQuake();		
	...
	private void onAutoButton()
	{
		...
		DrawEdit edit=plotView.getEdit();
		edit.clear();
		for(int i=0;i<ADJUST_LENGTH;i++){
	...
	private void onNextButton()
	{
		if(!saveData())
			return;
		Intent intent=new Intent(this,AdjustHeavyActivity.class);
